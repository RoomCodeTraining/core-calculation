...
use App\Mail\SendOpenedAssignmentNotificationMail;

class SendOpenedAssignmentNotificationJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $message = "Bonjour, nous avons bien reçu votre demande d'expertise pour le véhicule immatriculé ".$assignment->vehicle->license_plate.". Vous serez recontacté une fois les travaux effectués.";
        if($assignment->client){
            ...
            if($assignment->client->email){
                ...
                try {
                    Mail::to($assignment->client->email)->cc($user_emails)->send(new SendOpenedAssignmentNotificationMail($assignment));
                } catch (\Exception $e) {
                    Log::error($e);
                }
            }
